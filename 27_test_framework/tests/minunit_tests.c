...
{
    lib_function func = dlsym(lib, func_to_run);//load on sharedlib in memory
    check(func != NULL,
            "Did not find %s function in the library %s: %s",
            func_to_run, lib_file, dlerror());
    int rc =func(data);
    check(rc == expected, "Function %s returned %d, not %d for data:%s",
            func_to_run, rc, expected, data);
    return 1;
error:
    return 0;
}

char *test_dlopen() //check .so file is opend properly
{
